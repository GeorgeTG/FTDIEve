...
#ifndef __FTDI_EVE_H__
#define __FTDI_EVE_H__
...
class FTDIEve {
	public:
		...
		void begin(uint8_t CSpin, SPIClass* mSPI, uint8_t options = (EVE_CALIBRATE | EVE_TRIM) );
...
class Poly {
	...
	void perim() {
		for (byte i = 0; i < n; i++)
			EVE->Vertex2f(x[i], y[i]);
		EVE->Vertex2f(x[0], y[0]);
	}
	public:
	void begin(FTDIEve* EVE) {
		this->EVE = EVE;
		restart();

		EVE->ColorMask(0,0,0,0);
		EVE->StencilOp(KEEP, INVERT);
		EVE->StencilFunc(ALWAYS, 255, 255);
	}
	...
	void paint() {
		...
		y1 = min(16 * 480, y1);
		EVE->ScissorXY(x0, y0);
		EVE->ScissorSize(x1 - x0 + 1, y1 - y0 + 1);
		EVE->Begin(EDGE_STRIP_B);
		perim();
	}
	void finish() {
		EVE->ColorMask(1,1,1,1);
		EVE->StencilFunc(EQUAL, 255, 255);

		EVE->Begin(EDGE_STRIP_B);
		EVE->Vertex2f(0*16, 0*16);
		EVE->Vertex2f(800*16, 0*16);
	}
	void draw() {
		paint();
		finish();
